...
export default function MetallicPaint({
    imageSrc, seed = 42, scale = 4, refraction = 0.01, blur = 0.015, liquid = 0.75, speed = 0.3,
    brightness = 2, contrast = 0.5, angle = 0, fresnel = 1, lightColor = '#ffffff', darkColor = '#000000',
    patternSharpness = 1, waveAmplitude = 1, noiseScale = 0.5, chromaticSpread = 2, mouseAnimation = false,
    distortion = 1, contour = 0.2, tintColor = '#feb3ff'
}: MetallicPaintProps) {
    ...
    useEffect(() => {
        ...
        const light = hexToRgb(lightColor), dark = hexToRgb(darkColor), tint = hexToRgb(tintColor);
    ...
    useEffect(() => {
        ...
        rafRef.current = requestAnimationFrame(render);
        return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); canvas.removeEventListener('mousemove', handleMouseMove); };
    }, [ready, textureReady]);

    // Pass any additional props (like className) to the canvas element
    // BUT the interface above doesn't have className, so I should just return the canvas as is or wrap it.
